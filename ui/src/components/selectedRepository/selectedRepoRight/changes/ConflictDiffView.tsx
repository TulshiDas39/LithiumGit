    ...
    const getLineElems = ()=>{
        ...
        for(let i =0 ;i< props.lines.length;i++){
            ...
            if(line.text === undefined){        
                const child = startOfConflict ? <input id={`${props.side}_${line.conflictNo}`} type="checkbox" defaultChecked={!!line.taken} /> : <br />;                
                elems.push(<p key={i} className="d-flex justify-content-end w-100"> {child} </p>)
            }
            else{
                const checkBox = startOfConflict ? <span className="flex-grow-1 text-end"><input id={`${props.side}_${line.conflictNo}`} type="checkbox" defaultChecked={!!line.taken} /></span>  : null;
