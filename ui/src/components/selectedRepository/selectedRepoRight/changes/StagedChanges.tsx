import { EnumChangeListViewMode, EnumChangeType, EnumDiffViewMode, IFile, RepositoryInfo, StringUtils } from "common_library";
...
function StagedChangesComponent(props:IStagedChangesProps){    
    ...
    const displayStagedReadOnly = (file:IFile)=>{
        return IpcUtils.getGitShowResultOfStagedFile(file.path).then(res=>{
            const lines = StringUtils.getLines(res.result!);
            const options = ["--staged","--diff-algorithm=minimal",file.path];
            return IpcUtils.getDiff(options).then(diff=>{
            ...
            if(store.selectedFile!.changeType !== EnumChangeType.DELETED){
                if(store.selectedFile?.changeType === EnumChangeType.MODIFIED){
                    if(store.diffViewMode === EnumDiffViewMode.Unified){
                        displayStagedReadOnly(store.selectedFile).then(()=>resolve(true));
                        return;
                    }
    ...
    <div className="d-flex justify-content-between hover py-1" style={{height:40}}
     >
